...
# 데이터 로드
gene_embeddings = torch.load('./input_0307/gene_embeddings.pt')
drug_embeddings = torch.load('./input_0307/drug_embeddings.pt')
drug_graph_dict = torch.load('./input_0307/drug_graph_dict.pt')
drug_masks = torch.load('./input_0307/drug_mask_dict.pt')
labels_dict = torch.load('./input_0307/response_label_dict_LN.pt')
...
# 전체 데이터 필터링
train_gene_emb, train_drug_emb, train_drug_graphs, train_masks, train_labels = filter_data(
    train_indices, gene_embeddings, drug_embeddings, drug_graph_dict, drug_masks, labels_dict
)
val_gene_emb, val_drug_emb, val_drug_graphs, val_masks, val_labels = filter_data(
...
full_dataset = {
    'train': {
        'gene_embeddings': train_gene_emb,
        'drug_embeddings': train_drug_emb,
    ...
    'validation': {
        'gene_embeddings': val_gene_emb,
        'drug_embeddings': val_drug_emb,
...
torch.save(full_dataset, './dataset/train,val_dataset.pt')
...
for fold_num, (train_idx, val_idx) in enumerate(kf.split(train_val_indices)):
    ...
    fold_train_gene_emb, fold_train_drug_emb, fold_train_drug_graphs, fold_train_masks, fold_train_labels = filter_data(
        current_train_indices, gene_embeddings, drug_embeddings, drug_graph_dict, drug_masks, labels_dict
    )

    fold_val_gene_emb, fold_val_drug_emb, fold_val_drug_graphs, fold_val_masks, fold_val_labels = filter_data(
        current_val_indices, gene_embeddings, drug_embeddings, drug_graph_dict, drug_masks, labels_dict
    )
    ...
    fold_dataset = {
        'train': {
            ...
            'drug_graphs': fold_train_drug_graphs,
            'drug_masks': fold_train_masks,
        ...
        'validation': {
            ...
            'drug_graphs': fold_val_drug_graphs,
            'drug_masks': fold_val_masks,
